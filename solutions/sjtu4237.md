# SJTU 4237
显然暴力不可能通过，我们需要某种支持区间操作的数据结构。

主要的问题在于不好处理“插入元素”以及“比较元素”这两个操作。但是我们发现插入元素有一个比较好的性质，就是只在序列的尾部加入，不会修改已经加入的元素。这就比较容易联想到用 Hash 来处理。

这里用的是多项式 Hash（如果你不知道这是什么，就看[这里](https://oi-wiki.org/string/hash/)），因此只需要处理区间加、区间乘两种运算。线段树能够很好的实现这两种运算，因而用线段树实现。
